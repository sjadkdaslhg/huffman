...
std::shared_ptr<Node> buildTree(const std::vector<unsigned long long>& counts) {
    ...
    std::vector<std::shared_ptr<Node>> nodes(256);
    for (int i = 0; i < 256; i++)
        nodes[i] = std::make_shared<Node>(static_cast<unsigned char>(i), counts[i], nullptr, nullptr);
    std::priority_queue<std::shared_ptr<Node>> priority_queue{nodes.begin(), nodes.end()};
    for (int i = 0; i < 255; i++) {
